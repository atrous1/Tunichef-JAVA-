...
private void Rep (ActionEvent event) {
    try {
        // Créez une instance de la classe AjouterRéponse
        AjouterRéponse ajouterReponseController = new AjouterRéponse();

        // Passez l'ID sélectionné à la classe AjouterRéponse
        ajouterReponseController.setReclamationId(selectedReclamationId);

        // Récupérez la scène actuelle depuis le bouton
        Scene currentScene = ((Node) event.getSource()).getScene();

        // Chargez la vue AjouterRéponse dans un Parent
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/ajouterR.fxml"));
        Parent root = loader.load();

        // Remplacez la racine de la scène actuelle par la nouvelle vue
    ...
    private void genererPDF() {
        ...
        if (selectedReclamation != null) {
            try {
                ...
                PDImageXObject borderImage = PDImageXObject.createFromFile("C:\\xampp\\htdocs\\Integration\\src\\main\\resources\\Assets\\CadrePDf.jpg", document);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            ...
            reclamation.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
                if (newValue != null) {
                    selectedReclamationId = newValue.getIdRec(); // Assumez que votre Reclamation a une méthode getIdRec()
                }
            });
