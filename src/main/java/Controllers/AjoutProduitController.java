...
public class AjoutProduitController {
    ...
    void ajouterp(ActionEvent event) throws SQLException, IOException {
        ...
        if (selectedMenu != null) {
            idMenu = selectedMenu.getId_menu();
        } else {
            displayErrorAlert("Erreur de menu", "Veuillez sélectionner un menu.");
            return;
        }

        int id = MenuCardController.r.getId_menu();
        List<Menu> menuList = sm.rechMenu(id);
        if (menuList.isEmpty()) {
            displayErrorAlert("Erreur de menu", "Aucun menu trouvé avec l'ID spécifié.");
            return;
        }
        ...
        Produit produit1 = new Produit(nomProduit, descriptionProduitText, url_image, Integer.parseInt(prixProduitText), idMenu);
